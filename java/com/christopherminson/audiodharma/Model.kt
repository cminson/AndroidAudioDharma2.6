...
class Model {
    ...
    fun downloadAndConfigure(path: String) {
        ...
        //val test = "https://www.virtualdharma.org/AudioDharmaAppBackend/Config/CONFIG00.ZIP"
        try {
    ...
    suspend fun downloadMP3(talk: TalkData) : Boolean {
    ...
    fun reportTalkActivity(type: ACTIVITIES, talk: TalkData) {
        ...
        val url = URL_REPORT_ACTIVITY + '?' + parameters
...
*/
    ...
    fun getAlbums(content: String, query: String): ArrayList<AlbumData> {
        ...
        if (query != "") {
            ...
            var filter = query.lowercase(Locale.getDefault())
            for (album in albumList) {

                val searchData = album.Title.lowercase(Locale.getDefault())
    ...
    fun getTalks(content: String, query: String): ArrayList<TalkData> {
        ...
        if (query != "") {
            ...
            var filter = query.lowercase()
            for (talk in talkList) {

                val notes = TheDataModel.getNoteForTalk(talk.FileName).lowercase()
                val searchData = talk.Title.lowercase() + " " + talk.Speaker.lowercase() + " " +
                        talk.Date + " " + talk.Keys.lowercase() + " " + notes
    ...
    fun doesTalkHaveTranscript(talk: TalkData): Boolean {
        ...
        println("AD: TRANCRIPT - " + talk)
        if (talk.PDF.lowercase().contains("http:")) return true
        if (talk.PDF.lowercase().contains("https:")) return true
        println("AD: RETURNING FALSE NO TRANSCRIPT")
        return false
    }
    ...
    fun isFullURL(url: String): Boolean {
        ...
        if (url.lowercase().contains("http:")) return true
        if (url.lowercase().contains("https:")) return true
        return false
    }
